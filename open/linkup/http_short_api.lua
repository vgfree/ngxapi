...
----cfg         hostinfo [host] and [port] tab 
----data        send data 
----if_recv     recv flag
----max_timeout  default 2000ms

function http(cfg, data, if_recv,max_timeout)
    perf.bind("HTTP SHORT SUCCESS", "HTTP SHORT FAILURE")
    local tcp = socket.tcp()
    if tcp == nil then
        only.log("E", 'load tcp failed')
        perf.over("HTTP SHORT FAILURE")
        return nil
    end
...
function http_ex(cfg, data, if_recv, pro_name , pro_ignore_time)
    ...
    if ERRORINFO_TAB[pro_name] then
        if (tonumber(ERRORINFO_TAB[pro_name]) or 0) > 0 and  ngx.time() -  (tonumber(ERRORINFO_TAB[pro_name]) or 0 ) < pro_ignore_time then
    ...
    if tcp == nil then
        only.log("E", 'load tcp failed')
        perf.over("HTTP SHORT FAILURE")
        ERRORINFO_TAB[pro_name] = ngx.time()
        return nil
    end
