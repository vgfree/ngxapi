...
local function new_connect(sqlname)
	...
	--[[
	local ok = OWN_MYSQL_POOLS[sqlname]:settimeout(2)
	only.log("I", "set timeout : " .. (ok and "OK" or "NO"))
	]]--
...
local function mysql_api_procedure(db, sql)
	local res,err = db:execute(sql)
	if not res then
		-->> when sql is error or connect is break.
		only.log("E", "%s",string.gsub(err,"%%",""))
...
local function mysql_cmd(sqlname, cmds, ...)
	...
	repeat
		...
		if not stat then
			local l = string.format("%s |--->FAILED! %s do reconnect . . .", cmds , index)
			only.log("E", l)
...
function cmd(sqlname, ...)
	...
	perf.bind("MYSQL POOL SUCCESS", "MYSQL POOL FAILURE")
	if not fetch_pool(sqlname) then
		perf.over("MYSQL POOL FAILURE")
		return false,nil
	end
	...
	perf.over("MYSQL POOL SUCCESS")
	return true,ret
end


local function mysql_get_connection( sqlname )
	local env = assert(luasql.mysql())
	if not env then return false end
	
	local CLIENT_MULTI_STATEMENTS = 2^16
	local mysql_info = APP_LINK_MYSQL_LIST[sqlname]
	local ok, db = pcall(env.connect, env, mysql_info['database'],mysql_info["user"], mysql_info["password"], mysql_info["host"], mysql_info["port"], CLIENT_MULTI_STATEMENTS )
	env:close()

	if not ok or not db then
		only.log("E","mysql_get_connection failed, %s, %s " , mysql_info['database'] , mysql_info['host'])
		return false
	end
...
local function mysql_procedure(sqlname, cmds, ...)
	local mysql_api_list = {
		PROCEDURE =  mysql_api_procedure,
	}
	----------------------------
	-- start
	----------------------------
	...
	local db = mysql_get_connection(sqlname)
	if not db then
		return false
	end
	...
	repeat
		if conn then
			stat,ret = pcall(mysql_api_list[ cmds ], db , ...)
		end
		if not stat then
			local l = string.format("%s |--->FAILED! %s do reconnect . . .", cmds , index)
			only.log("E", l)
		...
		conn = mysql_get_connection(sqlname)
...
function exec(sqlname, ...)
	...
	if type(...) == 'table' then
		...
		for i=1,#... do
			...
			stat,ret[i] = pcall(mysql_procedure, sqlname, unpack((...)[i]))

			if not stat then err = ret[i] break end
		end
	else
		stat,ret = pcall(mysql_procedure, sqlname, ...)
		if not stat then err = ret end
	end

	-- only.log('D', string.format("END MYSQL CMD |---> %f", socket.gettime()))
	if not stat then
		only.log("E", "failed in mysql_procedure " .. tostring(err))
		perf.over("MYSQL POOL FAILURE")
		return false,nil
	end
	perf.over("MYSQL POOL SUCCESS")
